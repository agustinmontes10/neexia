    ...
    >
      <div className="max-w-[1280px] mx-auto">
        <Reveal className="max-w-[640px] mx-auto mb-16 text-center">
          <p className="text-[13px] font-bold uppercase tracking-[0.08em] text-brand-light mb-4">
            Cómo trabajamos
          </p>
        ...
        <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          <div className="hidden lg:block absolute left-0 right-0 top-5 h-[3px] rounded-full">
            <div className="relative h-full w-0 rounded-full bg-gradient-to-r from-brand-light/60 to-brand-light animate-proceso-fill">
              <span className="absolute top-1/2 right-0 w-2.5 h-2.5 rounded-full bg-brand-light animate-proceso-tip-glow" />
            </div>
          </div>
          {steps.map((step, i) => (
            <Reveal
            ...
            >
              <div
                className={`w-fit text-4xl font-extrabold text-[#EDEDED] bg-white tracking-[-0.02em] ${STEP_ANIMATIONS[i] ?? ""}`}
              >
                {step.n}
              </div>
